...
/*   Updated: 2025/03/21 16:48:50 by paulmart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (this->_hitPoint > 0)
	{
		...
		std::cout << "ClapTrap : " << this->_name << " has taken " << amount << " damage(s) and has " << this->_hitPoint << " hit point(s) left" << std::endl;
	}
	else
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	{
		if(this->_energyPoint < 1)
			std::cout << "ClapTrap : " << this->_name << " doesn't have enough energy point to be repaired" << std::endl;
		else
		{
			this->_energyPoint--;
			this->_hitPoint += amount;
			std::cout << "ClapTrap : " << this->_name << " has been repaired of" << amount << " hit point(s), he has now " << this->_hitPoint << " hit points" << std::endl;
		}
	}
	else
		std::cout << "ClapTrap : " << this->_name << " has 0 hit point so he can't be repaired anymore" << std::endl;
}